...
namespace db\scorers {
    function all() {
        ...
        $scorers = array();
        $res = $db->query('SELECT s.name AS name, s.goals AS goals, t.name AS team, t.abbr AS team_abbr FROM scorers s INNER JOIN teams t ON s.team = t.name ORDER BY goals DESC, name ASC');
